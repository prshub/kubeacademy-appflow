pipeline {
  ...
  environment {
    ...
    DOCKER_HUB_ACCOUNT = 'pramodsanaga'
  }
  stages {
      ...
      steps {
        container('docker') {
            dir ("site") {
                ...
                sh ("docker tag ${DOCKER_HUB_ACCOUNT}/${DOCKER_IMAGE_NAME}:${env.BUILD_NUMBER} ${DOCKER_HUB_ACCOUNT}/${DOCKER_IMAGE_NAME}:${env.BUILD_TAG}-latest")
                sh ("docker push ${DOCKER_HUB_ACCOUNT}/${DOCKER_IMAGE_NAME}:${env.BUILD_TAG}-latest")
            }
        }
      }
    }
